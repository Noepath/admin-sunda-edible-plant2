...
Route::middleware('auth.session')->group(function () {
    ...
    Route::get('/proxy/plants', function () {
        ...
        try {
            ...
            ])->get('https://stapin.site/api/v1/plants');

            if ($response->successful()) {
                return response()->json($response->json());
            } else {
                \Log::error('Plants API error: ' . $response->status() . ' - ' . $response->body());
            ...
            \Log::error('Plants API exception: ' . $e->getMessage());
            return response()->json(['error' => 'API request failed: ' . $e->getMessage()], 500);
        }
    })->name('proxy.plants');

    Route::get('/proxy/plant-categories', function () {
        if (!session('authenticated') || !session('firebase_token')) {
            return response()->json(['error' => 'Unauthorized'], 401);
        }
        ...
        try {
            $response = Http::timeout(30)->withHeaders([
                ...
                'Content-Type' => 'application/json',
                'Accept' => 'application/json'
            ])->get('https://stapin.site/api/v1/plant-categories');

            if ($response->successful()) {
                return response()->json($response->json());
            } else {
                \Log::error('Plant categories API error: ' . $response->status() . ' - ' . $response->body());
                return response()->json([
                    'error' => 'Failed to fetch plant categories data',
                    'status' => $response->status(),
                    'message' => $response->body()
                ], $response->status());
            }
        } catch (\Exception $e) {
            \Log::error('Plant categories API exception: ' . $e->getMessage());
            return response()->json(['error' => 'API request failed: ' . $e->getMessage()], 500);
        }
    })->name('proxy.plant-categories');

    // Route::get('/proxy/diseases', function () {
    //     if (!session('authenticated') || !session('firebase_token')) {
    //         return response()->json(['error' => 'Unauthorized'], 401);
    //     }
